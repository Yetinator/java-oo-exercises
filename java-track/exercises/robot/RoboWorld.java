...
public class RoboWorld {
	//Robot menu from I/O day 3 prep
		protected Robot[] world;
		protected String[] RobotTypes = {"Robot", "PushBot", "AttackBot"};

		//Constructor
		public void RoboWorld(){
			this.world = new Robot[100];
		 ...
		 * @return the world
		 */
		public Robot[] getWorld() {
			return world;
		}
		
		public Robot getWorld(int pos) {
			Robot bot = this.world[pos];
			return bot;
		}

		/**
		 * @return the robotTypes
		 */
		 ...
		 */
		public void setWorld(Robot[] world) {
			this.world = world;
		}
		public void setWorld(Robot bot, int pos) {
			this.world[pos] = bot;
		}
		...
		protected int printMenu(String[] choices){
		...
		protected int getInput(int menuLength){
		...
		protected String getStringInput(String feed){
		...
		protected boolean runAction(int selection){
			boolean exit = false;
			switch(selection){
			case 1:
				makeBot();
		...
		public void makeBot(){
			...
			int a = 1;
			/*
			int typeNum = 1;//getInput(this.RobotTypes.length);
			
			
			
			String Name = "Brian";//this.getStringInput("What is the name of your robot?");
